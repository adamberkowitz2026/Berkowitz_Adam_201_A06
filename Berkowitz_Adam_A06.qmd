...
[https://github.com/adamberkowitz2026/Berkowitz_Adam_201_A06](https://github.com/adamberkowitz2026/Berkowitz_Adam_201_A06)
...
![](Screen Shot 2024-05-22 at 3.18.16 PM.png)
...
for (i in names(player_info)) {
  points_per_game[[i]] <- player_info[[i]][[4]] / player_info[[i]][[3]]
}
...
```{r}
ppg <- as.vector(round(as.numeric(unlist(points_per_game)), digits = 2))
ppg
```
...
```{r}
for (i in 1:nrow(mat_rand)) { #instead of the method from the hint i used this iteration method from lecture notes 20
  for (j in 1:ncol(mat_rand)) {
    if (mat_rand[i,j] < 100) {
      mat_rand[i,j] = round(mat_rand[i,j], digits = -1)
    } else if (mat_rand[i,j] < 1000) {
      mat_rand[i,j] = round(mat_rand[i,j], digits = -2)
    } else {
      mat_rand[i,j] = round(mat_rand[i,j], digits = -3)
    }
  }
}
...
```{r}
max_temps
just_temps <- subset(max_temps, select = -c(state, year))
just_temps
mean(unlist(just_temps))
```
...
```{r}
CO_just_temps <- subset(CO_temps, select = -c(state, year))
median(unlist(CO_just_temps))
```
```{r}
median_temp <- apply(just_temps, 1, median)
print(paste0("The maximum median annual temperature of ", median_temp[which.max(median_temp)], " occurred in ", max_temps$year[which.max(median_temp)], " and the minimum median annual temperature of ", median_temp[which.min(median_temp)], " occurred in ", max_temps$year[which.min(median_temp)], "."))
```
